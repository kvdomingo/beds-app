...
async def check_in_patient(
    ...
    return await q.patients.check_in_patient(name=name, bed_id=bed_id)


@router.post("/{id}", response_model=Patient)
async def check_out_patient(id: str, q: Queriers = Depends(get_queriers)):
    return await q.patients.check_out_patient(id=id)
...
async def move_patient(
    ...
    return await q.patients.move_patient(id=id, bed_id=bed_id)
